...
dir_input="/project2/gilad/joycehsiao/dropseq-pipeline/genotypes_18489_phase3"
dir_temp="/project2/gilad/joycehsiao/dropseq-pipeline/genotypes_18489_phase3_tmp"
dir_pseudo="/project2/gilad/joycehsiao/dropseq-pipeline/genotypes_18489_pseudo_phase3"
...
# Rules:
#  1. presence of ancestral allele
#  2. individual genotype is not the same as the major/alterante combo
...
# keep sites if major/minor allele is ancestral
cat $dir_temp/chr${chr_num}.genotypes.ancestral.1.txt |\
  awk '{print toupper($0)}' |\
  awk '{ if (($5==$7) OR ($6==$7)) {print}}' \
  > $dir_temp/chr${chr_num}.genotypes.ancestral.2.txt

# keep sites if genotyped allele pair is not the same
# as the reference/alternate pair
cat $dir_temp/chr${chr_num}.genotypes.ancestral.2.txt |\
  awk '{ if( !(($3==$5) && ($4==$6)) && !(($3==$6) && ($4==$5)) ) {print}}' |\
...
cat $dir_temp/chr${chr_num}.genotypes.ancestral.3.txt | \
  awk '{print $1,$2}' > $dir_temp/chr${chr_num}.genotypes.ancestral.positions.txt

# subset vcf
vcftools --gzvcf $dir_input/chr${chr_num}.genotypes.18489.vcf.gz \
  --positions $dir_temp/chr${chr_num}.genotypes.ancestral.positions.txt \
  --recode --out $dir_temp/chr${chr_num}.pseudo
...
cat $dir_temp/chr${chr_num}.pseudo.recode.main.vcf | \
...
#  awk 'BEGIN{print "#CHROM","POS","ID","REF","ALT","QUAL","FILTER","INFO","FORMAT","NA18489","AA"}1' \
